...
struct ContentView: View {
  ...
  var body: some View {
    NavigationView {
      ZStack {
        Color.clear  // Add a clear view to add space at the top, Spacer() wont work
        ...
        HStack(spacing: 12) {
          ...
          ForEach(pages) { item in
            Image(item.thumbnailName)
              ...
              .onTapGesture(perform: {
                isAnimating = true
                pageIndex = item.id  // get item.id from ForEach
              })
          ...
          .opacity(isAnimating ? 1: 0)
          .frame(width: 260)
          .padding(.top, UIScreen.main.bounds.height / 12) // UIScreen.main.bounds.height will return actual hight of screen
          .offset(x: isDrawerOpen ? 20 : 215) // NOTE: remember x or y
